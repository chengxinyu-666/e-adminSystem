    ...
    <avue-crud :option="option"
      ...
      <template #menu="{row,index,size}">
        <div style="display: grid;grid-template-columns: repeat(1, auto);align-items: center;align-content: space-between;justify-items: end;justify-content: center;">
          <order_button v-bind:order="row" @start="start" @end="end" v-bind:type="'uncollection'"></order_button>
        </div>
      </template>
...
<script>
  ...
  import { getList} from "@/api/audit/ordercollection";
  import {
    order_collection,
    order_first_trial,
    order_note_list,
  ...
  export default {
    ...
    data() {
      return {
        ...
        loading: true,
        test:"userInfo",
        orderLabels:["no","userInfo","goodsInfo","status","rentInfo","depositMoney","note","distribute"],
        page: {
          pageSize: 10,
          currentPage: 1,
          total: 0
