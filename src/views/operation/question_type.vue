<template>
  <basic-container>
    <avue-crud :option="option"
      ...
      <template slot-scope="scope" slot="menu">
        <el-button type="text"
                   size="small"
                   icon="el-icon-link"
                   @click="showDialogPage(scope.row,'question')">问题列表
        </el-button>
      </template>
    ...
    <template>
      <el-dialog :title="questionPain.title"
                 :visible.sync="questionPain.visible"
                 width="80%"
                 append-to-body="true"
  ...
  export default {
    data() {
      return {
        ...
        form: {},
        selectionList: [],
        questionPain: {title: "", visible: false},
        loading: true,
        option: {
          tip: false,
          border: true,
    ...
    methods: {
      rowSave(row, done, loading) {
        row.code = "question_type";
        row.parentId = "1585202623076737025";
        add(row).then(() => {
          done();
      ...
      showDialogPage(row, type) {
        ...
        if (type == 'question') {
          this.questionPain.visible = true;
          this.questionPain.title = row.dictValue + " - 问题列表";
        }
      },
      onLoad() {
        this.loading = true;
        dictionary("question_type").then(res => {
